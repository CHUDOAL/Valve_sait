...
Перед деплоем обязательно добавьте переменные окружения на Render.com!

**Смотрите файл [ENV_VARIABLES.md](ENV_VARIABLES.md) для подробных инструкций.**

### Быстрый старт:
1. Добавьте переменную `OPENAI_API_KEY` в настройках Render
2. Получите ключ на [platform.openai.com](https://platform.openai.com)
